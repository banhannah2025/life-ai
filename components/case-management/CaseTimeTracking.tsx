...
export function CaseTimeTracking() {
  ...
  const hoursByAuthor = useMemo(() => {
    ...
    timeEntries.forEach((entry) => {
      map.set(entry.author, (map.get(entry.author) ?? 0) + entry.hours);
    });
      ...
      <header className="space-y-3">
        ...
        {!hasCases ? (
          <p className="rounded-md border border-dashed border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-500">
            Add a client and matter before logging time so entries can be associated correctly.
          </p>
        ) : null}
      </header>
                ...
                {entries.length ? (
                  entries.map((entry) => (
                    <TableRow key={entry.id}>
                      ...
                      <TableCell className="align-top text-sm text-slate-600">{entry.hours.toFixed(1)}</TableCell>
                      <TableCell className="align-top">
